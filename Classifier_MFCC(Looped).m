...
% same as Classifier_MFCC.m except set up to loop with different number of
% mixture components
...
precentages = zeros(1,10);
IncorrMales = zeros(1,10);
IncorrectFemales = zeros(1,10);

all = zeros(1,10,3);

for NUM_MIXTURES = 1:10
    ...
    precentages(NUM_MIXTURES) = Precentage;
    IncorrMales(NUM_MIXTURES) = IncorrectMale;
    IncorrFemales(NUM_MIXTURES) = IncorrectFemale;
    
end

all(:,:,1) = precentages;
all(:,:,2) = IncorrMales;
all(:,:,3) = IncorrectFemales;